...
for anomaly in anomalies:
    print(
        f"Row {anomaly['index']}: {anomaly['department']}, {anomaly['location']}, {anomaly['level']}"
    )
    print(
        f"  Probabilities: dept={anomaly['dept_prob']:.3f}, loc={anomaly['loc_prob']:.3f}, level={anomaly['level_prob']:.3f}"
    )
```
...
data["combined_prob"] = data.apply(
    lambda row: dept_probs[row["department"]]
    * location_probs[row["location"]]
    * level_probs[row["level"]],
    axis=1,
)
...
### Working with Contamination

The contamination parameter is just a percentile cutoff—it labels the top X% worst-scoring points as anomalies. This is convenient but crude.

Better approaches:

- **Use raw scores, not labels**: Skip the contamination parameter entirely. Export anomaly scores and set thresholds based on inspection or business requirements rather than guessing prevalence
- **Find natural gaps**: Plot the score distribution and look for visible breaks between normal and anomalous populations rather than arbitrary percentiles
- **Iterate with feedback**: Start conservative (contamination=0.01), review flagged cases, adjust upward based on false positive tolerance
- **Domain-driven thresholds**: Base cutoffs on acceptable risk levels (e.g., "we can manually review 50 cases per day") rather than expected anomaly rates you don't actually know
...
PartialDependenceDisplay.from_estimator(
    clf, X, features=[0, 1], feature_names=["age", "salary"]
)
plt.show()
```
